__meta__ = {
    "name": "OZON库存同步",
    "description": "按照客户数据源执行 OZON 与 ME3 库存同步",
    "entrypoint": "run",
    "configs": {
        "ME3_url": {
            "type": "string",
            "label": "ME3 库存接口地址",
            "description": "ME3 商品库存查询接口的完整地址",
            "required": True,
        },
        "ME3_app_key": {
            "type": "secret",
            "label": "ME3 App Key",
            "description": "ME3 分配的接口授权 Key",
            "required": True,
        },
        "ME3_secret": {
            "type": "secret",
            "label": "ME3 Secret",
            "description": "ME3 分配的接口授权密钥",
            "required": True,
        },
        "OZON_api_key": {
            "type": "secret",
        ...
        "OZON_client_id": {
            ...
            "description": "OZON Seller Client ID",
            "required": True,
        },
        "limit_Warehouse_id": {
            "type": "string",
            "label": "限定仓库编号",
            "description": "只处理该 OZON 仓库的库存记录",
            "required": True,
        },
        "item_list_url": {
            "type": "string",
            "label": "商品列表接口",
            "required": True,
            "default": "https://api-seller.ozon.ru/v4/product/info/stocks",
        },
        "item_info_url": {
            "type": "string",
            "label": "商品详情接口",
            "required": True,
            "default": "https://api-seller.ozon.ru/v3/product/info/list",
        },
        "stocks_by_warehouse_url": {
            "type": "string",
            "label": "仓库库存接口",
            "required": True,
            "default": "https://api-seller.ozon.ru/v1/product/info/stocks-by-warehouse/fbs",
        },
        "stocks_update_url": {
            "type": "string",
            "label": "库存更新接口",
            "required": True,
            "default": "https://api-seller.ozon.ru/v2/products/stocks",
        },
    },
    "data_source": {
        ...
        "required": True,
        "extensions": [".xlsx"],
        "description": "库存同步控制表，至少包含 sku 和 store_code 两列",
    },
}
...
def run(configs: dict, ctx: FeatureExecutionContext):
    ...
    platform_data_source = ctx.get_data_source_path()
    data_source_file_path = str(platform_data_source) if platform_data_source else os.path.join(current_dir, 'dataSource.xlsx')
