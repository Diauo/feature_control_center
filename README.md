...
功能控制中心是面向多客户的 Python 功能运行平台。当前版本 `2.3.0` 支持业务员按菜单授权、功能与用户删除（保留历史运行与审计记录、可重新上传恢复），以及由业务脚本主动提交通用执行结果报表：运行记录可直接核对总数、成功数和失败数，详情页以中文展示逐条结论并导出 XLSX。单次报表明细默认上限为 60000，管理员可在系统设置中调整；平台仍不解释商品模型，也不接管失败项重试。
...
python -m pip download --only-binary=:all: --dest wheelhouse backend/dist/feature_control_center-2.3.1-py3-none-any.whl

fcc-update-package build \
  --wheelhouse wheelhouse \
  --private-key /secure/offline/update-signing-private.pem \
  --output feature-control-center-2.3.1-linux-x86_64.fcup \
  --version 2.3.1 \
  --compatible-from '>=2.3.0,<3.0.0' \
...
上面是从本版 `2.3.0` 制作后续版 `2.3.1` 的发布示例；制作实际版本时，必须同步替换 wheel 文件名、`--output`、`--version`、`--compatible-from` 和 `--database-revision`，不得直接照抄版本号。
